...
| **10** | **V8 · draft autosave unload flush + signed-out badge** ⚠ lane | no `beforeunload`/`pagehide` flush (hard refresh in the 1.5s debounce window loses edits); signed-out `/add-recipe` shows a persistent "Couldn't save draft" error (BACKLOG UX, off the B1 #281 review) | `[~]` `feat/v8-draft-autosave-unload-flush` (2026-07-10) | `src/pages/AddRecipe/useDraftAutosave.ts`, `DraftSaveStatus.tsx` | needs `sendBeacon`/fetch-keepalive for unload (must carry B5's `updatedAt` precondition); auth-aware copy |
