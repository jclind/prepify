...
| **10** | **V1 · server `servingPrice` recompute** | server trusts client `servingPrice` + accepts `ingredientData:null` rows unchecked — B1's gate is client-only (BACKLOG Bugs, off the B1 #281 review) | `[~]` `feat/v1-server-servingprice-recompute` (2026-07-10) | `server/routes/recipes.js` (addRecipe/editRecipe) | recompute price server-side from submitted rows (null rows stay publishable by design) |
...
| **10** | **V5 · legacy rating-type migration script** | string `rating`/`reviewCreatedAt` docs interleave wrong under the Top sort (Mongo BSON type order) (BACKLOG Bugs, out of §D) | `[~]` `feat/v5-rating-type-migration` (2026-07-10) | `server/scripts/` (new) + Jest test | same ops-script shape as `reconcileRatingAggregates.js`; dry-run default, owner-gated `--apply` |
