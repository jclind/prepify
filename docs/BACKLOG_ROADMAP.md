...
| **10** | **V5 · legacy rating-type migration script** | string `rating`/`reviewCreatedAt` docs interleave wrong under the Top sort (Mongo BSON type order) (BACKLOG Bugs, out of §D) | `[P]` [#293](https://github.com/jclind/prepify/pull/293) (2026-07-10) | `server/scripts/normalizeRatingTypes.js` (new) + Jest test | PR open — dry-run default, `--apply` gated, exit-2-on-pending; null/garbage/idempotency covered; Jest 859 green. ⚠ **Cutover sequencing filed:** `addReview` still writes string `reviewCreatedAt`, so the prod `--apply` must land together with the write-path numeric flip or the New sort regresses |
