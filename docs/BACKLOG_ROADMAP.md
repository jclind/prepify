...
| **10** | **V1 · server `servingPrice` recompute** | server trusts client `servingPrice` + accepts `ingredientData:null` rows unchecked — B1's gate is client-only (BACKLOG Bugs, off the B1 #281 review) | `[x]` [#292](https://github.com/jclind/prepify/pull/292) (2026-07-10) | `server/routes/recipes.js` (addRecipe/editRecipe) | **merged** — new `server/util/calculateServingPrice.js` mirrors the client util (one deliberate NaN-guard divergence, commented); client value ignored on create+edit; edit path falls back to stored `servings` when omitted; `backfillServingPrice.js` deduped onto the shared helper; Jest 852 green |
| **10** | **V2 · admin reports pagination guard** | uncoerced/uncapped `page`/`perPage` → `limit(NaN)` 500s, negative skip, collection dump (BACKLOG Bugs, test-quality audit) | `[x]` [#289](https://github.com/jclind/prepify/pull/289) (2026-07-10) | `server/routes/reports.js` | **merged** — coercion/clamp/cap mirrors reviews.js verbatim + 4 pagination tests; orchestrator reverted two out-of-scope punctuation/copy edits (`d45db94`) during review |
| **10** | **V3 · draft-cap TOCTOU** | 25-draft cap is `countDocuments`-then-`insertOne`; concurrent POSTs at 24 exceed it (BACKLOG Bugs, test-quality audit) | `[x]` [#291](https://github.com/jclind/prepify/pull/291) (2026-07-10) | `server/routes/drafts.js` | **merged** — post-insert trim (keep-oldest-25, 409 `DRAFT_LIMIT` if the insert didn't survive; no single-doc write lock applies since drafts are separate docs); concurrency test fails-before/passes-after, 5x flake-free, Jest 849 green. Filed: `collections.js` has the same cap TOCTOU |
| **10** | **V4 · moderation blocklist unicode fold** | homoglyph/fullwidth evasion (Cyrillic-с `fuсk`→`fuk`, fullwidth `ｓｈｉｔ`→``) bypasses `normalizeToken` (BACKLOG Bugs, test-quality audit) | `[x]` [#288](https://github.com/jclind/prepify/pull/288) (2026-07-10) | `server/util/moderationBlocklist.js` | **merged** — NFKC + curated Cyrillic/Greek confusables fold; key find: the fold must run **before `tokenize()`** (an un-folded homoglyph splits the word apart); Jest 851 green |
| **10** | **V5 · legacy rating-type migration script** | string `rating`/`reviewCreatedAt` docs interleave wrong under the Top sort (Mongo BSON type order) (BACKLOG Bugs, out of §D) | `[x]` [#293](https://github.com/jclind/prepify/pull/293) (2026-07-10) | `server/scripts/normalizeRatingTypes.js` (new) + Jest test | **merged** (script is the deliverable; prod `--apply` owner-gated) — dry-run default, `--apply` gated, exit-2-on-pending; null/garbage/idempotency covered; Jest 859 green. ⚠ **Cutover sequencing filed:** `addReview` still writes string `reviewCreatedAt`, so the prod `--apply` must land together with the write-path numeric flip or the New sort regresses |
...
| **11** | **T1 · InstructionItem edit double-submit** | Enter fires `handleEditSubmit`, then the trailing manual `blur()` synchronously re-fires it via `onBlur` — the identical shape V6 fixed in `IngredientItem.tsx` (BACKLOG Bugs, off the V6 #287 lane) | `[x]` [#295](https://github.com/jclind/prepify/pull/295) (2026-07-10) | `src/pages/AddRecipe/Instructions/InstructionItem/InstructionItem.tsx` + `IngredientItem.tsx` (hardening) | **merged** — ref-guard mirror of [#287](https://github.com/jclind/prepify/pull/287), fails-before pins; orchestrator review found a latent stuck-flag edge in the #287 pattern itself (genuine click-away leaves the flag set → next session's click-away swallowed; jsdom-masked), fixed in BOTH components with reset-on-edit-entry + real-focus two-session regression tests. Vitest 734 green |
...
| **13** | **F1 · drafts.js per-uid rate limiter (POST-only)** | `drafts.js` has NO `makeUserLimiter` mount at all — but ⚠ not copy-paste: `PUT /drafts/:id` is the 1.5s-debounce autosave path (~40 legit writes/min) and #294's keepalive unload flush must never eat a 429 (BACKLOG Tech debt, off the U1 [#299](https://github.com/jclind/prepify/pull/299) lane) | `[x]` [#302](https://github.com/jclind/prepify/pull/302) (2026-07-11) | `server/routes/drafts.js`, `server/middleware/writeLimiter.js` (+ limiter tests) | **merged** — the pre-made design call verbatim: `draftWriteLimiter` (30/min default) on `POST /drafts` only, after `verifyToken → requireActive`; PUT deliberately unlimited with the autosave/#294-flush rationale as an in-code comment, pinned by a no-limiter-on-PUT wiring test + a >30-PUTs-all-pass test; wiring distinct-instances 4→5; the 30-cap test correctly handles `DRAFT_LIMIT` 409s interleaving below the rate cap. Jest 41/883 green. API_CONTRACT.md drafts section updated at close-out (429 + pre-existing #286 drift fixed: PUT's `updatedAt` precondition/`DRAFT_CONFLICT` 409 was undocumented, stale line refs) |
| **13** | **F2 · modernize numeric-0 servings test** | `recipeFormValidation.test.ts` passes `servings: 0` (a number) — unreachable under the post-#298 string contract; #300 kept it byte-identical behind a commented `@ts-expect-error` to stay typing-only (BACKLOG Tests, off the U2 [#300](https://github.com/jclind/prepify/pull/300) lane) | `[x]` [#301](https://github.com/jclind/prepify/pull/301) (2026-07-11) | `src/test/recipeFormValidation.test.ts` | **merged** — the numeric-0 case rewritten in place against reachable inputs (`''` → required, `'0'` → whole-number message, exact strings read from the source); commented `@ts-expect-error` dropped; one file, 4+/7− lines. Both tsc programs 0; Vitest at the exact 735/2 baseline (single case, two assertions, mirroring the sibling negative/fractional case) |
...
- **2026-07-09** — **Wave 7 landed** — all three lanes merged to `development` (owner-approved, after a local
  review pass): **W1** #268 (merge `2c8bf1e`), **W2** #269 (`25a6b11`), **W3** #270 (`d342471`); all six required
  checks green on each, merged in order with no conflicts, remote branches deleted, worktrees torn down. Board
  rows flipped `[P]`→`[x]`; matching BACKLOG.md boxes ticked (W1 Tech-debt migration script; W2 two A11y
  follow-ups; W3 CI-actions bump + `ReleaseNotes` version import). Review verified the risk areas by hand: W1's
  convert is transactional with the `findOne` inside the txn and `tls`-block identical to the six sibling ops
  scripts (prod `--apply` remains **owner-gated**); W2's new `aria-describedby`/`aria-errormessage` refs are each
  gated on the same `errors.*` that renders their `error-*` target, so no dangling references; W3's
...
- **2026-07-10** — **B1 claimed** (`feat/b1-addrecipe-enrichment-draft-safety`). Claim recorded directly on
  `development` (same convention as S4–S7) so concurrent sessions see the lane taken. Scope: the Wave-9
  AddRecipe safety lane — **M3** (gate submit while any ingredient row's enrichment is in flight, so a create
  can't persist `ingredientData:null` + an understated `servingPrice` forever; `useRecipeForm.ts` /
  `IngredientsContainer.tsx`) and **D3** (pre-title content is never autosaved — draft creation gated on
  `canCreateDraft = !!title.trim()`; trigger on any substantive content or warn on unload, preserving the
  no-stray-keystroke-drafts intent; `useDraftAutosave.ts`). Both share the AddRecipe file surface → one lane.
...
- **2026-07-10** — **B4 merged** ([#284](https://github.com/jclind/prepify/pull/284), merge `05659be`). Both
  ...
  `0.95`→"1", `1.96`→"2" instead of snapping to "7/8"; sub-threshold `1.9`→"1 7/8" unchanged. **I3** — added
  `formatIngredientQuantity()` as the single display entry point (renders `min–max` when a distinct upper
  bound was captured, else the single quantity, else '') and routed SingleRecipe, PrintableRecipe, and the
  add-recipe row through it; `updateIngredients` scales `minQty`/`maxQty` alongside `quantity` so a range
  rescales with servings (2–3 → 4–6). Folded in during review: (a) `formatIngredientQuantity` renders '' for a
  falsy (0/null) quantity, matching the prior truthiness guards — caught by /verify as a "0 salt" regression
  before merge; (b) a narrow range whose bounds round to the same fraction collapses to the single value
  instead of reading "1–1". Verified: gates green (`tsc`, Vitest, build) + runtime-verified headless against
  the dev client+API (ranges, rollover boundary both sides, servings rescale, zero-quantity rows). Left two
...
- **2026-07-10** — **Wave 10 batch one landed** (V2 [#289] merge `a322c6b`, V3 [#291] `6d3673d`, V4 [#288]
  `aa266d9`, V6 [#287] `56df0d4`, V7 [#290] `47847a2` — all CI-green, merged serially by the orchestrator
  after per-PR diff review; owner approved the merge policy for green Wave-10 PRs). First orchestrated
  wave: one model-matched subagent per lane in an isolated worktree, stop-at-PR-open, merges/board-flips
  serialized here. **Quality read:** Sonnet lanes strong (V4 found the pre-`tokenize()` fold requirement
  the filing missed; V3 justified post-insert-trim over a guarded write; V6 proved its pin by stash-out;
  V7 navigated the FormInput string-at-runtime gotcha). Haiku (V2) fixed the bug correctly but made two
  out-of-scope punctuation/copy edits (reverted in `d45db94`) — future mechanical lanes stay on Sonnet.
  **Process lesson recorded:** three agents initially wandered into the MAIN checkout instead of their
  worktrees (two self-corrected verifiably; V2 didn't notice) — batch two prompts now open with a hard
  `git rev-parse --show-toplevel` discipline check. Follow-ups filed in BACKLOG.md: `InstructionItem`
  double-submit sibling (V6), `collections.js` cap TOCTOU sibling (V3), `FormInput` unchecked-cast
  type-safety (V7). Worktrees/branches torn down.
- **2026-07-10** — **Wave 10 batch two: V1 + V5 landed** (V1 [#292] merge `32f78c4`, V5 [#293] `21a9d22`;
  CI-green, diff-reviewed, merged under the owner-approved green-Wave-10 policy). **V1** (Sonnet):
  `server/util/calculateServingPrice.js` mirrors the client util with one deliberate, commented divergence
  (the client's `<= 0` guard misses NaN — comparisons with NaN are false — so the server uses
  `!(Number(s) > 0)`); create+edit both recompute and ignore the client value; edit falls back to stored
  `servings` when the body omits it; `backfillServingPrice.js` deduped onto the shared helper. **V5**
  (Opus): `normalizeRatingTypes.js`, house ops-script shape (dry-run default, `--apply`, exit-2-on-pending
  as a cutover check); nulls/empties untouched, garbage reported never written, idempotent; 11 tests.
  **V5's key catch (filed):** `addReview` still WRITES string `reviewCreatedAt`, and the New sort works
  today *because* the field is uniformly string-typed — so the prod `--apply` and the write-path numeric
  flip must ship as one cutover step, in neither order alone. Both worktrees/branches torn down.
  **V8 (Opus) still in flight** — last batch-two lane.
- **2026-07-10** — **V8 merged ([#294] merge `3ae9313`) — Wave 10 fully drained** (8/8 lanes `[x]` in one
  day). V8 (Opus) closed both items: the `fetch`-keepalive unload flush (sync token cache off the axios
  interceptor + `warmAuth`; carries B5's `updatedAt`; bfcache-guarded) and the signed-out badge — the
  latter with a **stale-premise correction**: `/add-recipe` already sits in the `PrivateRoute` group, so
  the filed symptom is unreachable via routing; the calm badge shipped as defense-in-depth. Merge was
  gated on **live browser verification** (the keepalive-past-teardown claim is unprovable in jsdom):
  all four checks passed against the dev stack, and the verification **exposed + fixed a pre-existing B5
  bug** — mounting AddRecipe with `?draftId` (the drafts-list resume flow) never seeded `updatedAtRef`,
  so every autosave 409'd into the conflict latch; regression test proven failing on the old deps.
  **Wave 10 orchestration retro:** 8 lanes, 8 first-pass PRs, 0 rework rounds beyond one reverted
  scope-creep (Haiku/V2) and one directed verification pass (V8); three follow-ups + one cutover-sequencing
...
- **2026-07-10** — **Wave 11 batch one landed** (T2 [#296] merge `4ec54a8`, T1 [#295] `3d6a634`, T4 [#297]
  `c855126`; CI-green, diff-reviewed, merged under the owner-approved green-Wave-11 policy — re-confirmed
  via AskUserQuestion per the standing protocol). **T2** (Sonnet): post-insert trim adapted from V3 to the
  array-on-one-doc shape ($push position = commit order, keep-oldest-50, $pull overflow); filed the missing
  per-uid rate limiter on `POST /collections`. **T1** (Sonnet): ref-guard mirror of #287 — and the
  orchestrator's diff review caught a latent stuck-flag edge in the #287 pattern itself (a genuine
  click-away leaves the suppress flag set because the trailing self-`blur()` no-ops on an already-blurred
  element; the NEXT session's click-away submit is then swallowed and edit mode sticks open). jsdom masked
  it (`fireEvent.blur` doesn't move real focus), so the rework hardened BOTH `InstructionItem` and the
  already-merged `IngredientItem` with reset-on-edit-entry, pinned by two-session regression tests that
  move REAL focus (`element.focus()`), each proven failing pre-fix. Pattern note for future blur-wired
  handlers: prefer real-focus-movement tests over `fireEvent.blur`. **T4** (Sonnet): 404 standardization
  (deleteReview + the folded-in editReview case) and the submitLimiter house-JSON 429; the orchestrator
  added one commit striking the contract doc's stale pagination DRIFT bullet (fixed by #289). The
  `resolveBugReport` 409-without-`code` wart stays recorded in API_CONTRACT.md DRIFT (not re-filed). **T3
  claimed + dispatched** (batch two, Opus) — last lane of the wave. Ops note: a `gh pr checks --watch`
  poll tripped GitHub's secondary rate limit; switched to 60s-interval monitors.
- **2026-07-10** — **T3 merged ([#298] merge `2edf337`) — Wave 11 fully drained** (4/4 lanes `[x]`,
  boarded and landed the same day, same-day as Wave 10). T3 (Opus) chose the honest-string surface over a
  parse-prop design (a parse prop would relocate rather than delete the cast and touch ~11 string call
  sites): `FormInput` drops its generic, numeric consumers (`ServingsInput`, `TimeInput`) coerce at their
  own boundaries, `RecipeFormState.servings` is honestly `string` with `String()` at both hydration points.
  Orchestrator review pressure-tested the zero-runtime-change claim across persistence boundaries and it
  holds: the draft `draftContent` builder already `Number()`-coerced servings pre-PR (draft docs keep
  storing numbers), submit is pinned by a `typeof payload.servings === 'number'` assertion, hydrated
  strings render/validate/re-save identically. Future numeric misuse is a compile error, self-verified by
  `@ts-expect-error` type-tests — which had to live in a new `src/test-d/` because `src/test` is
  tsconfig-excluded; that type-coverage gap is filed, along with a sweep for other compound inputs hiding
  the TimeInput child-feeds-numeric-object shape. **Wave 11 retro:** 4 lanes, 4 PRs (#295–#298), one
  directed rework round (T1's stuck-flag hardening — the orchestrator diff review catching a latent bug in
  the already-merged #287 pattern was the wave's highest-value find), zero worktree-discipline violations
...
- **2026-07-10** — **U3 closed clean, no code change** — the compound-input audit (Sonnet, read-only)
  found **zero live instances** of the pre-#298 string-into-number lie: every `FormInput` consumer
  (13 checked), every raw `<input>` outside `FormInput`, and every compound aggregation point is
  honest-string state or coerces at the correct boundary (`SingleRecipe`'s servings stepper already
  mirrors the reference pattern). 9 sweep angles run and enumerated in the report; zero commits, clean
  worktree. Watch items recorded on the BACKLOG tick rather than filed (no action today): inert
  `RecipeFormState.fridgeLife`/`freezerLife` numeric fields with no UI control, and
  `Analytics.tsx`'s un-annotated button-driven `days`.
- **2026-07-10** — **U1 merged ([#299] merge `9fc5ad1`)** — `collectionWriteLimiter` (30/min default,
  justified vs the recipe surface's 12/min by no per-write paid cost) on `POST /collections` +
  `PATCH /collections/:id` with one shared bucket (the editRecipe/editReview convention); delete and
  membership-toggle left unlimited, matching every sibling delete/toggle route, with in-code rationale.
  Pinned by a real-429 `NODE_ENV`-flip test (mixed create+rename budget drain) + wiring-test cases; Jest
  879 green; diff-reviewed against the sibling mounts; merged on verified head `b51000a`, all 6 CI checks
  green. API_CONTRACT.md collections routes updated for the limiter/429. Lane finds filed in BACKLOG.md:
  **`drafts.js` mounts no limiter at all** (corrects this item's premise; flagged as NOT copy-paste — the
  draft PUT is the 1.5s-debounce autosave path and #294's unload flush must not eat a 429) and
  **collection names skip `moderateText`** (decide owner-private visibility first; may be won't-fix).
  Wave 12 remaining: U2 (typecheck-the-tests) still in flight.
- **2026-07-10** — **U2 merged ([#300] merge `2cb1fbe`) — Wave 12 fully drained** (U1/U2 merged + U3
  closed clean, boarded and landed the same day as Waves 10 and 11). The scope gate earned its keep:
  the agent measured first (1292 errors under a naive main-include — the Cypress config files inject a
  global Chai `expect` that shadows Vitest's; 17 honest errors; 12 after an ES2022 lib bump), paused at
  the decision gate with 9 clean fix-forwards and 3 judgment calls, and the orchestrator ruled: a
  type-level `getIngredientData` return narrowing in `src/api/recipes.ts` (authorized — zero runtime
  effect, fixes a real prod-type gap), `@ts-expect-error` over rewriting the unreachable numeric-0
  servings assertion (follow-up filed), and a declared `toHaveBeenCalledOnce`→`toHaveBeenCalledTimes(1)`
  swap. Also removed the vestigial pre-Vitest `@types/jest` (the suite had been typed against Jest's
  matcher shapes). CI's Static job now runs `tsc --noEmit -p tsconfig.tests.json`, so test-type drift
  fails CI. Merge gate held: diff verified typing-only, suite totals byte-identical (735/2), merged on
  verified head `ecb861c`, all 6 checks green. **Wave 12 retro:** 3 lanes → 2 PRs (#299, #300) + 1
...
- **2026-07-11** — **F1 + F2 merged — Wave 13 fully drained, board empty of code seeds.** F2
  ([#301](https://github.com/jclind/prepify/pull/301), merge `afbcd5b`, head `349920a`) and F1
  ([#302](https://github.com/jclind/prepify/pull/302), merge `5e56b6b`, head `6e1361d`), both
  diff-reviewed and merged after all 6 checks green on their exact head SHAs. Close-out: both
  BACKLOG.md items ticked; API_CONTRACT.md drafts section updated — the flagged POST 429 plus
  incidental pre-existing drift fixed (the PUT entry predated #286: its required `updatedAt`
  precondition, the `DRAFT_CONFLICT` 409 with the current-doc payload, and the delete-race
  404-not-200-null behavior were all undocumented, and the drafts handler line refs were stale;
  the "DRIFT — collections/drafts/gamification" summary's `200 null` claim corrected likewise).
  Teardown: both agent worktrees removed, lane branches deleted local+remote (the merged
  `worktree-agent-*` placeholders pruned with `-d`), `git branch -r --merged` swept clean.
  **Wave 13 retro:** 2 lanes → 2 PRs, zero worktree violations, zero rework rounds — both
  pre-made design calls (POST-only limiter; in-place assertion rewrite) came back implemented
