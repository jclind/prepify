...
| **8** | **X3 · orphaned-image-on-failed-create** | delete the uploaded Storage object when `POST /addRecipe` fails after the image upload (`src/api/recipes.ts`) | `[~]` worktree-feat+x3-orphaned-image-cleanup (2026-07-09) | `src/api/recipes.ts` | ready — overlaps X2 file surface (serialize or one lane) |
