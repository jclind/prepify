...
| **11** | **T3 · FormInput `setVal` type-safety** | generic `setVal(next as T)` hands the raw DOM string to callers regardless of `T` — numeric consumers carry a type that lies at runtime (BACKLOG Tech debt, off the V7 #290 lane) | `[~]` claimed 2026-07-10 (`feat/t3-forminput-setval-type-safety`) | `src/Components/Form/FormInput.tsx` + numeric call sites | batch two, dispatched after T1's merge (#295 landed; agent rebases on current development) |
