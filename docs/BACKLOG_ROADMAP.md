...
| **10** | **V2 · admin reports pagination guard** | uncoerced/uncapped `page`/`perPage` → `limit(NaN)` 500s, negative skip, collection dump (BACKLOG Bugs, test-quality audit) | `[P]` [#289](https://github.com/jclind/prepify/pull/289) (2026-07-10) | `server/routes/reports.js` | PR open — coercion/clamp/cap mirrors reviews.js verbatim + 4 pagination tests, Jest green; orchestrator reverted two out-of-scope punctuation/copy edits (`d45db94`) during review |
| **10** | **V3 · draft-cap TOCTOU** | 25-draft cap is `countDocuments`-then-`insertOne`; concurrent POSTs at 24 exceed it (BACKLOG Bugs, test-quality audit) | `[P]` [#291](https://github.com/jclind/prepify/pull/291) (2026-07-10) | `server/routes/drafts.js` | PR open — post-insert trim (keep-oldest-25, 409 `DRAFT_LIMIT` if the insert didn't survive; no single-doc write lock applies since drafts are separate docs); concurrency test fails-before/passes-after, 5x flake-free, Jest 849 green. Filed: `collections.js` has the same cap TOCTOU |
| **10** | **V4 · moderation blocklist unicode fold** | homoglyph/fullwidth evasion (Cyrillic-с `fuсk`→`fuk`, fullwidth `ｓｈｉｔ`→``) bypasses `normalizeToken` (BACKLOG Bugs, test-quality audit) | `[P]` [#288](https://github.com/jclind/prepify/pull/288) (2026-07-10) | `server/util/moderationBlocklist.js` | PR open — NFKC + curated Cyrillic/Greek confusables fold; key find: the fold must run **before `tokenize()`** (an un-folded homoglyph splits the word apart); Jest 851 green |
...
| **10** | **V6 · ingredient inline-edit double-submit** | Enter fires `handleEditSubmit` then `blur()` synchronously re-fires it via `onBlur` → double parse call (real proxy quota) + double toast (BACKLOG Bugs, off the B3 #282 review) | `[P]` [#287](https://github.com/jclind/prepify/pull/287) (2026-07-10) | `src/pages/AddRecipe/Ingredients/IngredientItem.tsx` | PR open — ref-guard skipping the self-triggered blur; pin verified by stash-out (2-calls without the fix); Vitest 708 green. Filed: `InstructionItem.tsx` carries the identical pattern (no network cost) |
| **10** | **V7 · client contract nits** | unencoded `filter`/`username` query interpolation (`recipes.ts:546,568`); negative/fractional `servings` accepted (`recipeFormValidation.ts:61`); `formatRating` renders literal `"NaN"`; + fold-in: `UserRatings` passes a sort the server ignores (BACKLOG Bugs + Tech debt) | `[P]` [#290](https://github.com/jclind/prepify/pull/290) (2026-07-10) | `src/api/recipes.ts`, `src/pages/AddRecipe/recipeFormValidation.ts`, `src/util/formatRating.ts`, `src/pages/Account/UserRatings/` | PR open — all 4 items fixed incl. `UserRatings` `'newAdd'`→`'new'` (server silently no-sorted unknown values); Vitest 712 green. Filed: `FormInput`'s generic `setVal` unchecked-cast type-safety follow-up |
