...
- **Personal cooking log ("you've cooked this N times", with dates)** — the "Made It" button is currently a **binary per-user marker**, not a log: the server stores `madeRecipes` as a `$addToSet` of `{ recipeId }` with *no dates*, and the recipe's public `numTimesMade` counter increments only on a user's *first* mark (`server/routes/recipes.js` — deliberately inflation-guarded), so there's no per-user cook history. A real cooking log — record each cook with a timestamp and surface "made N times · last cooked <date>" on the recipe and profile — is the missing **data foundation** that both the *time-based cooking streaks* and *"Made it!" photo upload* ideas below silently assume already exists. `[needs API]` (add a dated per-cook log — e.g. `madeRecipes[].madeAt[]` — and return it from `GET /checkMadeRecipe`; decide whether the public `numTimesMade` counts distinct cooks or distinct users). *(Filed 2026-07-08 — surfaced by the API-contract drift fix that aligned `MadeRecipeBtn` to today's binary model; the aspirational per-cook-date UI it used to carry was removed there. See the BACKLOG "checkMadeRecipe" item.)*
...
- **Time-based cooking streaks** — gamification today is count-based achievements. Adding "cooked 4 recipes this week" / "3 new cuisines this month" streaks gives a recurring reason to come back, distinct from the one-shot badges. **Depends on the *personal cooking log* above** — the server does *not* currently keep per-cook dates (`madeRecipes` is a dateless set), so the timestamp history this needs has to be built first.
...
- **"Made it!" photo upload + visual reviews** — `MadeRecipeBtn` records that a user made a recipe (once, per user); letting users optionally attach a photo turns text reviews into a gallery of real attempts. Pairs naturally with the *personal cooking log* above (a photo per cook wants the dated per-cook records that log would introduce). `[reuses the Firebase Storage upload flow]` `[needs API]` (extend the `madeRecipe` body + `GET /recipes/:id/made-photos`)
