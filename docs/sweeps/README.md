...
|---|---|---|---|---|
| **Performance** | 2026-06-26 | #198 | image `loading`/`decoding` deferral (Home cards), `RecipeCard` memo + `decoding`, trending `staleTime` | 6 → [BACKLOG → Tech debt](../BACKLOG.md#tech-debt--process--infra): route code-splitting, missing Mongo indexes, `/recipes/facets` scans, recipe-page CLS pop-in, `AuthContext` memo, image `srcset` |
