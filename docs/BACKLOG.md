...
- `[ ]` **`usePaginatedLoadMore` re-appends a page's items on refetch, duplicating saved-grid cards** *(from `sweeps/BUG_HUNT_2026-07-09.md` M8, filed 2026-07-10; boarded Wave 9 · B2)* — the accumulate effect is keyed on `data` alone and unconditionally appends for `page>0`, so any structurally-changed refetch of the observed page (default `QueryClient` → `refetchOnWindowFocus`; e.g. a rating on a page-1 recipe changes) re-appends the whole page: duplicate cards, duplicate `_id` keys, and an inflated length that corrupts `isMore`. Affects Saved / Ratings / Your Recipes and the SingleRecipe reviews list. `src/pages/Account/usePaginatedLoadMore.ts:89`. **Fix:** track which pages have been appended (or dedupe by `_id`) instead of keying the append on `data`. *(Distinct from the error/empty-conflation item below, fixed in [#274](https://github.com/jclind/prepify/pull/274).)*
