...
- **Request:** query `recipeId` (string, required). `400 {error:'recipeId is required'}`; `403 {error:'Review not found or not authorized'}` if the user has no doc for this recipe.
...
### GET /api/reports
- **Handler:** `server/routes/reports.js:178`
- **Middleware:** `verifyToken` → `requireAdmin` → `asyncHandler`
- **Request:** query: `status` (`'open' | 'resolved' | 'dismissed'`; invalid values silently ignored = unfiltered), `targetType` (`'recipe' | 'review' | 'user'`; invalid ignored), `page` (default `0`), `perPage` (default `20`). No clamping: `page`/`perPage` go straight through `parseInt` (reports.js:186-187) — a non-numeric value yields NaN and the Mongo cursor will throw (surfaces as `500`), unlike the clamped bug-reports twin.
...
- **Middleware:** `submitLimiter` (raw `express-rate-limit`, **15 requests / 15 min per IP**, standard headers, skipped when `NODE_ENV === 'test'`; the 429 body is the library default plain-text message, NOT the `{error, code: 'RATE_LIMITED'}` JSON of `makeUserLimiter`) → `optionalAuth` (sets `req.uid` when a valid Bearer token is present; anonymous otherwise) → `asyncHandler`
- **Request:** body:
...
- **GET /api/reports pagination is unclamped while its bug-reports twin clamps** — non-numeric `page`/`perPage` on `/api/reports` produce NaN skip/limit (reports.js:186-187) and a 500 from the Mongo cursor, whereas `/api/admin/bug-reports` clamps both (bugReports.js:104-105). Server-side asymmetry; the shipped client always sends numbers.
- Rate-limit response shapes differ between the two submit endpoints: `POST /api/reports` 429s with the `makeUserLimiter` JSON (`{error, code: 'RATE_LIMITED'}`), while `POST /api/bug-reports` 429s with express-rate-limit's default plain-text body (bugReports.js:33-39 sets no `message`). Neither client branches on 429, so no functional break — but callers can't assume a uniform 429 shape across these routes.
## Admin
